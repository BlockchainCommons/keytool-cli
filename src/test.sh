...
TEST_SEED_REQUEST_UR='ur:crypto-request/otadtpdagdpysghkadjycfgeemmwjkonhefxglrfdiaotaadwkoyadtaaohdhdcxzmoycylumhmdgwspnyvadaktnsoycwmyaodihgftdllugltphlmtutytadosdwwdaxksctgajtiyjljpjnhsjyinjljtcxhsidjlkpjycxjyisinjkcxjpihjskpihjkjydmgtjnytos'
...
TEST_FULL_ADDRESS_DERIVATION_PATH=${TEST_ACCOUNT_DERIVATION_PATH}/${TEST_ADDRESS_DERIVATION_PATH}
...
testFullAddressDerivationPath()
{
  ...
  assertEquals ${TEST_FULL_ADDRESS_DERIVATION_PATH} "$(${KEYTOOL} full-address-derivation-path)"
}
...
{
  assertEquals ${TEST_REQUEST_ID} \
    "$(${KEYTOOL} --key-request-id ${TEST_REQUEST_ID} key-request-id)"
}

testKeyRequestDescription()
{
  assertEquals "${TEST_REQUEST_DESC}" \
    "$(${KEYTOOL} --key-request-description "${TEST_REQUEST_DESC}" key-request-description)"
}

testKeyRequestType()
{
  assertEquals "private" \
    "$(${KEYTOOL} key-request-type)"

  assertEquals "private" \
    "$(${KEYTOOL} --key-request-type private key-request-type)"

  assertEquals "public" \
    "$(${KEYTOOL} --key-request-type public key-request-type)"
}

testKeyRequest()
{
  assertEquals "ur:crypto-request/otadtpdagdpysghkadjycfgeemmwjkonhefxglrfdiaotaadykoeadykaotaaddyoeadlecsghykaeykaeykaewkaewkaocyhngrmuwzaxksctgajtiyjljpjnhsjyinjljtcxhsidjlkpjycxjyisinjkcxjpihjskpihjkjydmeyrthfmw" \
    "$(${KEYTOOL} --key-request-id ${TEST_REQUEST_ID} --key-request-description "${TEST_REQUEST_DESC}" --master-key-fingerprint ${TEST_MASTER_KEY_FINGERPRINT} key-request)"

  assertEquals "ur:crypto-request/otadtpdagdpysghkadjycfgeemmwjkonhefxglrfdiaotaadykoeadykaotaaddyoeadlecsghykaeykaeykaewkaewkaocyhngrmuwzaxksctgajtiyjljpjnhsjyinjljtcxhsidjlkpjycxjyisinjkcxjpihjskpihjkjydmeyrthfmw" \
    "$(${KEYTOOL} --key-request-id ${TEST_REQUEST_ID} --key-request-description "${TEST_REQUEST_DESC}" --master-key ${TEST_MASTER_KEY_UR} key-request)"
}

TEST_KEY_REQUEST='ur:crypto-request/otadtpdagdpysghkadjycfgeemmwjkonhefxglrfdiaotaadykoeadykaotaaddyoeadlecsghykaeykaeykaewkaewkaocyhngrmuwzaxksctgajtiyjljpjnhsjyinjljtcxhsidjlkpjycxjyisinjkcxjpihjskpihjkjydmeyrthfmw'
TEST_KEY_RESPONSE='ur:crypto-'

testKeyResponse()
{
  assertEquals ${TEST_KEY_RESPONSE} \
    "$(${KEYTOOL} --key-request-id ${TEST_REQUEST_ID} --key-request-description "${TEST_REQUEST_DESC}" --source-key ${TEST_MASTER_KEY_UR} key-response)"
}

testDerivedKey()
{
  assertEquals ${TEST_ADDRESS_KEY_UR} \
    "$(${KEYTOOL} --key-request-id ${TEST_REQUEST_ID} --key-request-description "${TEST_REQUEST_DESC}" --source-key ${TEST_MASTER_KEY_UR} derived-key)"
}

testReadKeyRequest()
{
  assertEquals ${TEST_REQUEST_ID} \
    "$(${KEYTOOL} --key-request ${TEST_KEY_REQUEST} key-request-id)"

  assertEquals "${TEST_REQUEST_DESC}" \
    "$(${KEYTOOL} --key-request ${TEST_KEY_REQUEST} key-request-description)"

  assertEquals "private" \
    "$(${KEYTOOL} --key-request ${TEST_KEY_REQUEST} key-request-type)"

  assertEquals ${TEST_MASTER_KEY_FINGERPRINT}/${TEST_FULL_ADDRESS_DERIVATION_PATH} \
    "$(${KEYTOOL} --key-request ${TEST_KEY_REQUEST} key-request-derivation-path)"
}
...
testPSBTSignatureRequestID()
{
  assertEquals ${TEST_REQUEST_ID} \
    "$(${KEYTOOL} --psbt-signature-request-id ${TEST_REQUEST_ID} psbt-signature-request-id)"
}

testPSBTSignatureRequestDescription()
{
  assertEquals "${TEST_REQUEST_DESC}" \
    "$(${KEYTOOL} --psbt-signature-request-description "${TEST_REQUEST_DESC}" psbt-signature-request-description)"
}

testPSBTSignatureRequestMultisign()
(
  psbt_to_sign=''
  key1='cSyLgU8rSvYNU1j6XR2vo5ebSQqza7PR9iFNtkkYrFMwyB5or5gH'
  key2='cVok3VJ1fGhdT2yAndKHFWvgTbDsdVFXb8xE9rsAcQASuS4JV9HG'
  ...
  request1="$(${KEYTOOL} --psbt-signature-request-id ${TEST_REQUEST_ID} --psbt ${psbt_to_sign} psbt-signature-request)"
  assertEquals \
    "ur:crypto-" \
    "${request1}"

  response1="$(${KEYTOOL} --network testnet --address-ec-key-wif ${key1} --psbt-signature-request ${request1} psbt-signature-response)"
  assertEquals \
  "ur:crypto-" \
  "${response1}"

  psbt1="$(${KEYTOOL} --psbt-signature-response ${response1} psbt)"
  assertEquals \
  ...
  request2="$(${KEYTOOL} --psbt-signature-request-id ${TEST_REQUEST_ID} --psbt ${psbt1} psbt-signature-request)"
  response2="$(${KEYTOOL} --network testnet --address-ec-key-wif ${key2} --psbt-signature-request ${request2} psbt-signature-response)"
  psbt2="$(${KEYTOOL} --psbt-signature-response ${response2} psbt)"

  request3="$(${KEYTOOL} --psbt-signature-request-id ${TEST_REQUEST_ID} --psbt ${psbt2} psbt-signature-request)"
  response3="$(${KEYTOOL} --network testnet --address-ec-key-wif ${key3} --psbt-signature-request ${request3} psbt-signature-response)"
  psbt3="$(${KEYTOOL} --psbt-signature-response ${response3} psbt)"

  signed_transaction="$(${KEYTOOL} --psbt ${psbt3} transaction-hex)"
  assertEquals "${final_transaction}" "${signed_transaction}"
)

# Eat all command-line arguments before calling shunit2.
shift $#
if [ "$(uname)" == "Darwin" ]; then
. ../deps/shunit2/shunit2
